...
@login_required(login_url='findbus')
def feedback(request,new={}):
    context = {}
    if request.method == 'POST':
        username_r = request.POST['username']
        email_r = request.POST['email']
        comments_r = request.POST['comments']
        feed = Feedback(user_name=username_r,email_Id=email_r,comments=comments_r)
        feed.save()
